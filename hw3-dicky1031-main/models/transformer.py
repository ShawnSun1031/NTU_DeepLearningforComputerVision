...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if self.return_intermediate:
            return output, torch.stack(inter_weight)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt = self.norm3(tgt)
        return tgt,self.with_pos_embed(tgt, query_pos),self.with_pos_embed(memory, pos)
    ...
    def forward_pre(self, tgt, memory,
        ...
        tgt2,self_weight = self.self_attn(q, k, value=tgt2, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)
        tgt = tgt + self.dropout1(tgt2)
        tgt2 = self.norm2(tgt)
        multi_attn,weight = self.multihead_attn(query=self.with_pos_embed(tgt2, query_pos),
                                   key=self.with_pos_embed(memory, pos),
                                   value=memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)
     
        tgt = tgt + self.dropout2(multi_attn)
        tgt2 = self.norm3(tgt)
