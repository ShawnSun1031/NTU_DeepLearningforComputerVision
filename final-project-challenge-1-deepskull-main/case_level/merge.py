...
def merge_clf_yolos(kind):
    ...
    df_coord=pd.read_csv("pred/merge_yolos_%s.csv"%(kind))
    out = "id,label,coords\n"
    ...
    with open("pred/merge_yolos_clf_%s.csv"%(kind),"w") as f:
        f.write(out)
mergeyolos("pred/0.62_test.csv","pred/0.66_test.csv","pred/0.69_test.csv",out_path="pred/merge_yolos_test.csv")
